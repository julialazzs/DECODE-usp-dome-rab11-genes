...
```{r}
usp_ctrl_combined <- readRDS(file="/g/huber/users/lazoriak/usp_ctrl_combined.rds")

# Extract raw counts and metadata to create SingleCellExperiment object
counts <- usp_ctrl_combined@assays$RNA@counts 

metadata <- usp_ctrl_combined@meta.data

# Set up metadata as desired for aggregation and DE analysis
#before this step, make sure object is annotated
metadata$cluster_id <- factor(usp_ctrl_combined$predicted.celltype)
metadata$cluster_id <- case_when (
  metadata$cluster_id %in% c("aEC", "pEC", "mEC", "LFC") ~ "EC",
  TRUE ~ as.character(metadata$cluster_id))

metadata$cluster_id %in% c("unk2", "unk1", "unk3") ~ NA_character_,
  T ~ metadata$cluster_id
  )

metadata$cluster_id <- factor(metadata$cluster_id)
  
metadata$sample_id <- factor(usp_ctrl_combined$orig.ident)
...
res <- results(dds, 
               name = "group_id_usp_vs_ctrl",
               alpha = 0.05)

# Shrink the log2 fold changes to be more appropriate using the apeglm method - should cite [paper]() when using this method
res <- lfcShrink(dds, 
                 coef = "group_id_usp_vs_ctrl",
                 res=res,
                 type = "normal")
```
...
y-axis - the negative log10 of the p-value associated with the statistical test for each gene. This transformation helps visualize the statistical significance of gene expression changes. Higher values indicate lower p-values and, thus, greater statistical significance. 